...
impl State {
    ...
    fn try_extract(&mut self) {
        ...
        let effective_patterns = merge_disabled(&self.config.patterns, &profile.disabled);
...
/// Returns a clone of `base` with `profile_disabled` merged into `disabled`.
fn merge_disabled(
    base: &config::PatternsConfig,
    profile_disabled: &std::collections::HashSet<String>,
) -> config::PatternsConfig {
    if profile_disabled.is_empty() {
        return base.clone();
    }
